...
def get_budget(nr_sources,nr_rows,nr_chans,nr_corrs,data_type,cb_args,fudge_factor=2.,row2source_ratio=100):
    ...
    if cb_args.model_chunks and cb_args.row_chunks:
        ...
        print('sources per chunk = {0:.0f} (user setting)'.format(sources_per_chunk))
        print('rows per chunk    = {0:.0f} (user setting)'.format(rows_per_chunk))
        ...
        print('sources per chunk = {0:.0f} (auto setting)'.format(sources_per_chunk))
        print('rows per chunk    = {0:.0f} (auto setting)'.format(rows_per_chunk))
        memory_usage = rows_per_chunk*sources_per_chunk*memory_per_row_source*nrthreads
        print('expected memory usage = {0:.2f} GB'.format(memory_usage/1024**3))
    else:
        print('For now you must set both row and source chunk, or leave both unset (=0); you cannot set only one of them.')
        sys.exit(1)
